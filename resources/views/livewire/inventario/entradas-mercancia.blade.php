      ...
      <section class="relative rounded-2xl md:rounded-3xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        ...
        <div class="relative px-4 py-5 md:px-6 md:py-6 space-y-6 md:space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            ">
              <label class="block text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha *</label>
              ...
              @error('fecha_contabilizacion') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            @foreach ($entradas as $index => $entrada)
              <div class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3">
                <div class="flex items-center justify-between mb-2">
                  <h4 class="text-sm font-semibold text-gray-800 dark:text-white">Ítem #{{ $index + 1 }}</h4>
                  <button type="button" wire:click="eliminarFila({{ $index }})"
                          class="text-red-500 hover:text-red-700" title="Eliminar"><i class="fas fa-trash-alt"></i></button>
                </div>

                {{-- Producto --}}
                <label class="block text-xs text-gray-500 mb-1">Producto</label>
                <input
                  list="productos_list_m_{{ $index }}"
                ...
                @error("entradas.$index.producto_id") <span class="text-red-600 text-[11px]">{{ $message }}</span> @enderror

                {{-- Descripción --}}
                <label class="block text-xs text-gray-500 mt-3 mb-1">Descripción</label>
                <input type="text" disabled
                       wire:model="entradas.{{ $index }}.descripcion"
                       class="w-full h-10 px-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-sm text-gray-700 dark:text-white" />

                <div class="grid grid-cols-2 gap-3 mt-3">
                  {{-- Cantidad --}}
                  <div>
                    <label class="block text-xs text-gray-500 mb-1">Cantidad</label>
                    <div class="flex items-center rounded-xl border border-gray-300 dark:border-gray-700 overflow-hidden">
                    ...
                    @error("entradas.$index.cantidad") <span class="text-red-600 text-[11px]">{{ $message }}</span> @enderror
                  </div>
                  ...
                  <div>
                    ...
                    @error("entradas.$index.bodega_id") <span class="text-red-600 text-[11px]">{{ $message }}</span> @enderror
                  </div>
                </div>
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       class="w-full h-10 text-center px-3 rounded-xl border
                              ...
                              @else border-gray-300 @enderror
                              dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-violet-600" />
                @error("entradas.$index.precio_unitario") <span class="text-red-600 text-[11px]">{{ $message }}</span> @enderror
              </div>
            @endforeach
                ...
                @foreach ($entradas as $index => $entrada)
                  <tr class="border-t dark:border-gray-700 hover:bg-violet-50 dark:hover:bg-gray-700 transition">
                    ...
                    <td class="p-3 align-top">
                      ...
                      @error("entradas.$index.producto_id") <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                    </td>
                    ...
                    <td class="p-3 text-center align-top">
                      ...
                      @error("entradas.$index.cantidad") <span class="text-red-600 text-xs block mt-1">{{ $message }}</span> @enderror
                    </td>
                    ...
                    <td class="p-3 text-center align-top">
                      ...
                      @error("entradas.$index.bodega_id") <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                    </td>
                    ...
                    <td class="p-3 text-center align-top">
                      <input id="fila-{{ $index }}-precio" type="number" step="0.01" min="0"
                             ...
                             class="w-[120px] h-10 text-center px-3 rounded-xl border
                                    ...
                                    @else border-gray-300 @enderror
                                    dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-violet-600" />
                      @error("entradas.$index.precio_unitario") <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                    </td>
